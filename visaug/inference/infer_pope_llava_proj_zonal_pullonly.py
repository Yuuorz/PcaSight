"""
LLaVA 只拉不推分区投影（proj_zonal_pullonly）：只做 +proj_vis，不做 -proj_sys。
...
class ZoneProjAdapter(AGVPAdapter):
    ...
    def forward(self, hidden_states, attention_mask=None, position_ids=None,
        ...
        if kv_seq_len > SYS_LEN + IMG_LEN and q_len > SYS_LEN + IMG_LEN:
            ...
            if imb > 0:
                fs = getattr(self, '_agvp_fixed_strength', None)
                if fs is not None:
                    strength = fs
                else:
...
def apply_zonal_projection(model, zone_config, fixed_strength=None):
    """
    zone_config: list of (start_layer, end_layer, strength_scale)
    fixed_strength: if set, use this constant strength instead of imb*|cos|*scale
    """
    for i, layer in enumerate(model.model.layers):
        ...
        adap._agvp_cos_gate = 'none'
        adap._agvp_mode = 'imb_cos_vis'
        if fixed_strength is not None:
            adap._agvp_fixed_strength = fixed_strength
        adap = adap.half().cuda()
        layer.self_attn = adap
        mode = f"fixed={fixed_strength}" if fixed_strength is not None else f"scale={scale}"
        print(f"  Layer {i}: proj ({mode})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fixed-strength", type=float, default=None,
                        help="If set, use this constant strength instead of imb*|cos|*zone_scale*multiplier")
    ...
    # Zone config: (start, end, strength_scale), scaled by strength_multiplier
    sm = args.strength_multiplier
    zone_config = [(2, 8, 0.5 * sm), (9, 16, 1.0 * sm), (17, 31, 1.0 * sm)]
    apply_zonal_projection(model, zone_config, fixed_strength=args.fixed_strength)
