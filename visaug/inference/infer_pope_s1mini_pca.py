...
if __name__ == "__main__":
    ...
    for line in tqdm(questions):
        ...
        # ==========================================================
        # 修正后的 Tokenize 与边界拦截数据流
        # ==========================================================
        # 1. 优先使用 apply_chat_template 生成纯文本 Prompt 字符串
        prompt_str = processor.apply_chat_template(
            messages, 
            add_generation_prompt=True, 
            tokenize=False, # 强行转为字符串范式，避开官方内核的不确定返回
            enable_thinking=False
        )

        # 2. 显式调用 processor 统一打包，转换为真正包含 tensors 的字典型 inputs
        inputs = processor(
            text=prompt_str, 
            images=image, 
            return_tensors="pt"
        ...
        # 3. 此时 inputs 必然是合法的字典，安全获取 input_ids
