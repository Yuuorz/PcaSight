...
class AttnAdapter(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # - 新版本 LlamaAttention.__init__(config, layer_idx)
        # - 老版本 LlamaAttention.__init__(config)
        try:
...
def attention_compute(attention, SYS_LEN=35, IMG_LEN=576):

    attention = torch.mean(attention, dim=1)
    assert len(attention.shape) == 3 and attention.shape[0] == 1
    # saliency shape (seq_len, seq_len)
    attention = attention.squeeze(0).float().numpy()
